...
void AutonomousMode::run (SystemState* s)
{
    ...
    std::thread pathPlanningThread(&PathPlanner::run, &pathPlanner, std::ref(inputImage), std::ref(outputImage), std::ref(lane), std::ref(trafficSigns), std::ref(obstacles) , std::ref(vehicle), std::ref(trajectory));
