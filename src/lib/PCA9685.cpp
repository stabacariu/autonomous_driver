 ...
 * Edited by        : Sergiu-Petru Tabacariu
 * Version          :
...
int PCA9685::openfd() {
    ...
    if ((fd = open(busfile, O_RDWR)) < 0) {
        ...
        std::cerr << "Couldn't open I2C Bus " << _i2cbus << " [openfd():open " << errno <<"]" << std::endl;
        return -1;
    }
    if (ioctl(fd, I2C_SLAVE, _i2caddr) < 0) {
        //~ printf ("I2C slave %d failed [openfd():ioctl %d]", _i2caddr, errno);
        std::cerr << "I2C slave " << _i2caddr << " failed [openfd():ioctl " << errno << "]" << std::endl;
        return -1;
    }
